...
class SearchQuerySet(object):
    ...
    def spatial(self, point, distance, location_field=None):
        """Narrows the search to results within a specified distance of
        the specified point. The optional location_field argument can be used
        to specify which index field determines an object's location. If not
        supplied, the first field found of type LocationField is used."""
        clone = self._clone()
        clone.query.add_spatial(point, distance, location_field)
        return clone
